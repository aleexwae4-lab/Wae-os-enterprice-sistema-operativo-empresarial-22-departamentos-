...
export default function AssetModule({department}:{department:Department}){
  ...
  const transferAsset=(assetId:string)=>{
    ...
    setAssignments(v=>[{id:`ASN-${String(v.length+1).padStart(2,'0')}`,assetId,from:asset.custodian,to:next,date:'01 sep 2026',reason:'Transferencia demo gobernada',status:'Activo' as const},...v.map(a=>a.assetId===assetId?{...a,status:'Histórico' as const}:a)])
