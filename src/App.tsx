...
import {
  Activity, Bell, Bot, BrainCircuit, Building2, ChevronDown, ChevronRight, CircleGauge, Command,
...
import {
  CognitiveCore, ExecutiveResponseCard, KnowledgeGovernance, MultimodalSupport, SectorContextPanel,
} from './EvolutionPanels'

type View = 'dashboard' | 'empresas' | 'departments' | 'projects' | 'analytics' | 'documents' | 'training' | 'platform' | 'workspace' | 'knowledge' | 'cognitive'
    ...
    <div className="app-shell">
      <aside className={`sidebar ${sidebarOpen ? 'open':''}`}>
        ...
        <button className="company-switcher" onClick={()=>{setView('empresas');setSidebarOpen(false)}}>
          ...
          <div><b>Aurora Dynamics</b><small>Tecnología · Multi-sector ready</small></div>
          <ChevronDown size={16}/>
        </button>
        ...
        <div className="sidebar-footer"><span className="status-dot"/> Cognitive Core activo <span>v1.1</span></div>
      </aside>

      <main className="main-area">
        <header className="topbar">
          <div className="top-left"><button className="icon-btn menu" onClick={()=>setSidebarOpen(true)}><Menu size={20}/></button>
            <div className="searchbox"><Search size={16}/><input placeholder="Buscar en empresa, memoria y documentos" value={query} onChange={e=>setQuery(e.target.value)}/><kbd>⌘ K</kbd></div>
          </div>
          <div className="top-actions"><span className="context-badge"><i/> tenant aislado</span><button className="icon-btn"><Bell size={18}/><i/></button><span className="ceo-badge">CEO</span></div>
        </header>
...
function Dashboard({onDepartment}:{onDepartment:(id:string)=>void}){
  return <>
    <PageHead eyebrow="CENTRO DE MANDO" title="Buenos días, Alex" subtitle="Operación ejecutiva, inteligencia corporativa y riesgos en una sola vista." />
...
function AiPanel({department,onClose}:{department:Department;onClose:()=>void}){
  const [input,setInput]=useState('')
  const [messages,setMessages]=useState<ChatMessage[]>([{role:'ai',text:`Soy ${department.agent}. Opero ${department.name.toLowerCase()} usando contexto empresarial, sectorial y evidencia autorizada.`}])
  const send=()=>{if(!input.trim())return;setMessages(v=>[...v,{role:'user',text:input},{role:'ai',text:'Contexto validado. Antes de ejecutar voy a comprobar datos faltantes, permisos, evidencia, impacto y dependencias entre departamentos.'}]);setInput('')}
  return <aside className="ai-panel"><div className="ai-head"><div className="ai-orb"><Sparkles size={18}/></div><div><b>WAE Intelligence</b><small>{department.agent} · {department.name}</small></div><button className="icon-btn" onClick={onClose}><X size={18}/></button></div><div className="ai-modes"><button className="active">Profundidad</button><button>Rápido</button><button>Evidencia</button><button>Acción</button></div><div className="messages">{messages.map((m,i)=><div key={i} className={`message ${m.role}`}><span>{m.text}</span></div>)}{messages.length>1&&<ExecutiveResponseCard department={department.name}/>}</div><div className="ai-suggestions"><button onClick={()=>setInput('Analiza el estado general de la empresa')}>Estado general</button><button onClick={()=>setInput('Detecta riesgos críticos y cita evidencia')}>Riesgos + evidencia</button></div><div className="composer"><button className="icon-btn"><Plus size={18}/></button><textarea placeholder="Pregúntale a WAE..." value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}}}/><button className="send" onClick={send}><Command size={16}/></button></div></aside>
}
