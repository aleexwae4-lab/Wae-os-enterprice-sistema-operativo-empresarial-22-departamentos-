...
async function guestFlow(req,res,body,profile,departmentId,input){
  ...
  try{let token=await startSession(req,key);let response=await upstream(req,key,profile,token,input,historyOf(body.history),'',departmentId);if(response.status===401||response.status===410){sessions.delete(key);token=await startSession(req,key);response=await upstream(req,key,profile,token,input,historyOf(body.history),'',departmentId)}return await relay(response,res,null,'ephemeral-real-ai')}catch(error){console.error('[Enterprise22 Expert Proxy]',error);return json(res,503,{ok:false,error:'cloud_expert_unavailable',retryable:true})}
}
