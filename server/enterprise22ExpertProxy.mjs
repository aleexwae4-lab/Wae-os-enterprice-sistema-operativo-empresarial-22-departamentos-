...
function contract(profile,userInput,privateContext=''){
...
async function privateFlow(req,res,body,profile,departmentId,input,token){
  ...
  try{
    ...
    let response=await upstream(req,key,profile,sessionToken,input,history,privateContext)
    if(response.status===401||response.status===410){sessions.delete(key);sessionToken=await startSession(req,key);response=await upstream(req,key,profile,sessionToken,input,history,privateContext)}
    ...
    await relay(response,res,{runtime:'private_ai',private_data:true,persistence:true,conversation_id:conversationId,company_name:context.company.name},'private-ai-v1')
...
async function guestFlow(req,res,body,profile,departmentId,input){
  ...
  try{let token=await startSession(req,key);let response=await upstream(req,key,profile,token,input,historyOf(body.history));if(response.status===401||response.status===410){sessions.delete(key);token=await startSession(req,key);response=await upstream(req,key,profile,token,input,historyOf(body.history))}return await relay(response,res,null,'ephemeral-real-ai')}catch(error){console.error('[Enterprise22 Expert Proxy]',error);return json(res,503,{ok:false,error:'cloud_expert_unavailable',retryable:true})}
}
